...
## Telling a stale cache from a stale deployment

Observed 2026-09-10, deleting a test race, and worth writing down mostly for
the diagnostic at the end.

Both races were gone from the database, from git and from the manifest, and
`/races/<slug>/data.json` answered 404 — but `/races/<slug>/` answered **200
from some requests and 404 from others**, for over an hour. Splitting by
`cf-ray` showed it was regional: IAD answered 404, EWR served a three-hour-old
copy with the `age` header climbing in real time. A fresh deployment did not
change it. Neither did a Purge Everything on the zone, at first glance.

At which point the shape of the evidence looked like a bad deployment, and it
was not. **Ask for a URL nothing has ever requested, and the answer is
unambiguous:**

    curl -o /dev/null -w '%{http_code}' \
      "https://sendoff.run/races/<slug>/?cb=$RANDOM"

404, from every host, every time. No cache anywhere can hold a URL that has
never been asked for, so this separates what is deployed from what is
remembered in one request. Everything above was cache.

Two things that made it confusing, both worth knowing:

- **Purging the zone does not touch `sendoff-abi.pages.dev`.** That name is not
  on the `sendoff.run` zone, so it kept serving its own stale copies while the
  apex was mostly clean. Comparing the two hosts looked like evidence of a
  broken deployment and was evidence of two caches.
- **Individual files expire independently**, so a race directory can answer 404
  for its config and 200 for its data at the same moment. That looks like a
  half-finished delete and is not.

The delete itself was complete throughout. What survives is an empty shell that
forwards into `race.html`, which asks the worker and is told the race is gone,
so no race data is reachable through it.

**The rule that follows:** the worker's `/public` is what says whether a delete
worked, because it is the read path. A static file is a copy, and copies lie
for as long as their cache says they may.
