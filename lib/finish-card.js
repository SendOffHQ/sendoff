...
(function () {
  ...
  // How wide the course profile is against its height. The stack shapes draw it
  // as a band, measured at 922x276 on a 4:5, and that is the proportion the
  // card was designed around. The split shapes used to give it whatever height
  // the right column had left, which came out at 1.4 on a 4:3 and 1.6 on a
  // 16:9: nearly square, and reading as a different design rather than the same
  // card in another frame. Capping it here makes the four shapes look related.
  // The height it gives up becomes slack, and slack is centred.
  const PROFILE_ASPECT = 3.3;
  ...
  function drawProfile(ctx, box, cfg, runner, c, gpxPts) {
    ...
    // The frame scales with the card. A constant stroke is proportionally
    // half as heavy on a 1920px 16:9 as on a 1080px 4:5, which is why the
    // wide shapes looked as though they had no frame at all.
    const frame = Math.max(3, Math.round(ctx.canvas.width / 360));
  ...
  async function render(canvas, opts) {
    ...
    if (ratio.mode === 'stack') {
      ...
      let profileH = show.profile
        ? Math.max(150, Math.min(760, roomR - (stH ? stH + 48 : 0),
                                Math.round(rightW / PROFILE_ASPECT)))
        : 0;
