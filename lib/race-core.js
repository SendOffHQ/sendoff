...
(function () {
  ...
  const gh = {
    ...
    // sat there with no map and no elevation until Pages caught up. Returns
    // null rather than throwing, because a race without a GPX is normal.
    async readRaceText(slug, file, preferWorker) {
      const cfg = auth.cfg();
      const path = `races/${slug}/${file}`;
      if (preferWorker && cfg) {
        try {
          const r = await gh.getFile(cfg, path);
          if (!r.missing) return r.content;
        } catch (e) { /* fall through to Pages */ }
      }
      try {
        const res = await netFetch(`races/${encodeURIComponent(slug)}/${file}?_=` + Date.now(), { cache: 'no-store' });
        if (res.ok) { offline.noteResponse(res); return await res.text(); }
      } catch (e) { /* fall through */ }
      if (cfg) {
        try {
          const r = await gh.getFile(cfg, path);
  ...
  const nav = {
    ...
    setRace(slug, current, canEdit) {
      ...
      if (typeof canEdit === 'function') {
        ...
        hub.load().then(() => nav._build(slug, current, !!fn())).catch(() => {});
        return;
      }
  ...
  AccessPanel.prototype.isCreator = function () {
    ...
    return !!me && ((this.cfg().createdBy || '').toLowerCase() === me);
  };
