...
(function () {
  ...
  const course = {
    ...
    aidStations(cfg) {
      ...
      const out = [{
        name: segs[0].fromAid || 'Start', mileage: 0,
        crewAccess: true, crewNote: null, pacerEligible: false, dropBag: false, checkpoint: false
      }];
      let cum = 0;
      for (const s of segs) {
        cum += (s.distanceMi || 0);
  ...
  function mountAccountWidget() {
    ...
    function render() {
      ...
      if (!signedIn) {
        // One thing to do signed out, so a menu to hold it would be a menu of one.
      ...
      // Four buttons and an address do not fit across a phone: the address got
      // cut to two characters and the buttons wrapped onto their own line above
      // the wordmark. Everything but who you are now lives behind one control.
      const email = s.email || s.username || 'account';
      const isAdmin = s.role === 'admin';
      bar.innerHTML =
        '<span class="account-dot"></span>' +
        '<span class="account-email"></span>' +
        '<button type="button" class="account-menu-btn" aria-haspopup="true" aria-expanded="false" ' +
          'aria-label="Account menu"><span class="account-bars"></span></button>' +
        ...
        '</div>';
      bar.querySelector('.account-email').textContent = email;
      // The full address, which the strip itself may have had to trim.
  ...
  const charts = {
    ...
    render(container, cfg, data) {
      ...
      for (const m of fuel.metrics(cfg)) {
        let target = m.targetPerHour;
        if (bands.length) {
          const steps = [];
          for (const b of bands) {
            const y = fuel.targetAt(cfg, m.key, b.fromHour);
            if (y == null) continue;
