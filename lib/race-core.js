...
(function () {
  ...
  const gh = {
    ...
    async readRaceFile(slug, file, preferWorker) {
      ...
      const keep = (text, at) => { if (text != null) lastSeen.save(slug, file, text, at); return text; };

      if (preferWorker && cfg) {
        try {
          const r = await gh.getFile(cfg, path);
          if (!r.missing) return keep(r.content);
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (res.headers.get('X-SendOff-Cache') !== 'hit') {
            offline.noteResponse(res);
            return keep(text);
          }
          const at = Date.parse(res.headers.get('date') || '') || 0;
          const saved = lastSeen.load(slug, file);
          if (saved && Date.parse(saved.at) > at) { offline.noteSaved(saved.at); return saved.text; }
          offline.noteResponse(res);
          return keep(text, at ? new Date(at).toISOString() : null);
        }
      ...
      const saved = lastSeen.load(slug, file);
      if (saved) { offline.noteSaved(saved.at); return saved.text; }
      return null;
    },
  ...
  const lastSeen = {
    ...
    // `at` is when this copy was made, for a copy that did not come off the
    // network just now. Never goes backwards: a stale copy arriving late must
    // not overwrite a fresher one this device already has.
    save(slug, file, text, at) {
      ...
      if (have && Date.parse(have.at) > Date.parse(stamp)) return;
      const row = JSON.stringify({ at: stamp, text });
      try {
    ...
    clear() {
      ...
      try { lastSeen.evictExcept(null); } catch (e) {}
    }
  };
  ...
  const offline = {
    ...
    // Set when a read fell all the way back to what this device saw last.
    // Same effect as a cache hit, and for the same reason: what is on screen
    // is not what is on the server, and saying so is the whole point of the
    // bar. The time is when the copy was taken, not when it was displayed.
    noteSaved(at) {
      offline.stale = true;
      const d = at ? new Date(at) : null;
      if (d && !isNaN(d)) offline.staleAt = d;
      offline.render();
    },
  ...
  AccessPanel.prototype.render = function () {
    const s = this.state;
    if (!s) return;
    const self = this;
    const cfg = this.cfg();
    ...
    const pageUrl = `${location.origin}${dir}race.html?id=${encodeURIComponent(this.slug())}`;
    ...
    this.body().innerHTML = `
      <div class="access-section">
        <div class="access-section-title">Display units</div>
        <div class="access-add">
          <select id="ax-unit-distance">
            <option value="mi"${u.distance === 'mi' ? ' selected' : ''}>Miles</option>
            <option value="km"${u.distance === 'km' ? ' selected' : ''}>Kilometers</option>
          </select>
          <select id="ax-unit-elevation">
            <option value="ft"${u.elevation === 'ft' ? ' selected' : ''}>Feet</option>
            <option value="m"${u.elevation === 'm' ? ' selected' : ''}>Meters</option>
          </select>
          <button class="btn primary" id="ax-units-save">Save units</button>
        </div>
        <div class="access-help">Sets how distance, elevation and pace display for everyone viewing this race. Stored data is unchanged.</div>
        <div id="ax-units-result"></div>
      </div>
      ...
      <div class="access-section">
        ...
        <div class="access-hint">${isPrivate
          ? 'This race is private, so this link only opens for people you have given access to. A view link below lets somebody watch without an account.'
      ...
      ${!this.isCreator() ? '' : `
        ...
        <div class="access-section danger-zone">
          <div class="access-section-title">Delete this race</div>
          <div class="access-help">
            Removes the course, the roster and every logged leg, and takes the race
            off the hub. Share links stop working. Only you can do this, crew cannot.
          </div>
          <div class="access-add">
  ...
  AccessPanel.prototype.confirmDelete = function () {
    const cfg = this.cfg();
    const data = this.ctx.data ? this.ctx.data() : null;
    const legCount = ((data && data.runners) || [])
      .reduce((n, r) => n + ((r.legs || []).length), 0);
    const name = cfg.name || this.slug();
  ...
  AccessPanel.prototype.wireHandlers = function () {
    ...
    const unitsSave = body.querySelector('#ax-units-save');
    if (unitsSave) unitsSave.addEventListener('click', async () => {
      const distance = body.querySelector('#ax-unit-distance').value;
      const elevation = body.querySelector('#ax-unit-elevation').value;
      unitsSave.disabled = true; unitsSave.textContent = 'Saving…';
      try {
        await gh.mutateJson(auth.cfg(), `races/${slug}/config.json`,
          (c) => { c.units = { distance, elevation }; return c; },
          `hub: set display units for ${slug}`);
        // Applied in place by the page. Re-reading config here would race
        // GitHub's own consistency and briefly show the pre-save units.
        const cfg = self.cfg();
        cfg.units = { distance, elevation };
        if (self.ctx.onUnitsSaved) self.ctx.onUnitsSaved({ distance, elevation });
        const out = body.querySelector('#ax-units-result');
        if (out) out.innerHTML = '<div style="color: var(--green); font-size: 11px; margin-top: 8px;">Saved ✓: units updated.</div>';
        unitsSave.disabled = false; unitsSave.textContent = 'Save units';
      } catch (err) {
        const out = body.querySelector('#ax-units-result');
        if (out) out.innerHTML = `<div class="access-err">${esc(err.message)}</div>`;
        unitsSave.disabled = false; unitsSave.textContent = 'Save units';
      }
    });

    const del = body.querySelector('#ax-delete');
    if (del) del.addEventListener('click', async () => {
      if (!await self.confirmDelete()) return;
      const out = body.querySelector('#ax-delete-result');
      del.disabled = true; del.textContent = 'Deleting…';
      out.innerHTML = '';
      try {
        const res = await api.deleteRace(slug);
        if (res && res.ok === false) {
          // A 207: some of it went, some did not. Say which rather than
          // claiming success and leaving the repo half tidied.
          const bits = [];
  ...
  // ---------- expose ----------
  window.Race = { fmt, units, slug, qs, hub, config, auth, share, api, gh, queue, roles, plans, archive, nav, course, stationAt, fuel, compute, gpx, charts, access, mountAccountWidget, offline };
})();
