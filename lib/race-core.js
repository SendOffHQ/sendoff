...
(function () {
  ...
  const gh = {
    ...
    async nextRaceId(cfg) {
      const digits = 6;
      if (cfg.mode === 'proxy') {
        const res = await fetch(`${cfg.proxyUrl}/next-race-id`, {
        ...
        (fuel.metrics(cfg).length
          ? '<div class="intake-grid">' +
              fuel.metrics(cfg).map(m =>
                `<div class="intake-card"><h3>${m.label}${m.unit ? ' ' + m.unit : ''} / hour</h3>` +
  ...
  const offline = {
    ...
    render() {
      ...
      const show = offline.stale || navigator.onLine === false;
      ...
      if (!el) {
        ...
        el.style.cssText =
          ...
          'text-align:center;background:#3A2A12;color:#FFB07A;border-top:1px solid #FFB07A;';
    ...
    register() {
      ...
      window.addEventListener('online', () => { offline.stale = false; offline.render(); });
  ...
  // ---------- expose ----------
  window.Race = { fmt, units, slug, qs, hub, config, auth, share, api, gh, course, stationAt, fuel, compute, gpx, charts, mountAccountWidget, offline };
})();
