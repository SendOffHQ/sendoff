...
(function () {
  ...
  function live(slug, onChange) {
    ...
    if (!hub.data() || !hub.data().live || !cfg || !cfg.proxyUrl) return state;

    function url() {
      const base = String(cfg.proxyUrl).replace(/^http/, 'ws').replace(/\/+$/, '');
  ...
  const gh = {
    ...
    async readPublic(path) {
      ...
      return j && j.content != null ? base64ToUtf8(j.content) : null;
    },
    ...
    async readRaceFile(slug, file, preferWorker) {
      ...
      // Signed out, and the caller asked for the freshest copy there is. For a
      // public race the worker has one that was written during the commit,
      // rather than at the next site rebuild twenty to thirty seconds later.
      // Anything not public answers 404 and falls through to the published
      // file below, which is still how an unlisted race is read.
      if (preferWorker && !cfg && !publicMiss.has(slug)) {
        try {
  ...
  function mountAccountWidget() {
    ...
    function render() {
      ...
      if (isAdmin) {
        api.feedbackCount(feedback.seen()).then((r) => {
  ...
  // ---------- expose ----------
  window.Race = { fmt, units, slug, qs, raceHref, poll, raceState, POLL_MS, live, hub, config, auth, share, api, gh, queue, roles, plans, archive, activities, nav, course, stationAt, fuel, compute, gpx, charts, access, mountAccountWidget, mountBetaBadge, offline, lastSeen, feedback, openFeedbackModal };
})();
