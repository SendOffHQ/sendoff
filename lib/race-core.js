...
(function () {
  ...
  // onChange is called with the message the worker sent, { type, slug, file,
  // at }, or null when there was nothing to parse. A caller that ignores the
  // argument behaves exactly as it did before there was one.
  function live(slug, onChange) {
    let ws = null, closed = false, tries = 0, timer = null, proxy = null;
    const state = { connected: false, close };

    // Waited for, not read. hub.json says whether there is a push at all and
    // where it lives, and it arrives over the network: this used to read
    // hub.data() on the spot and give up when it was still null, with no
    // retry. pit.html and racer.html call this from inside an async function,
    // after awaits, so theirs had landed. race.html calls it at the end of its
    // script, in the same tick it starts, so the race page has never once
    // opened a socket. It polled instead, which is why nothing looked broken:
    // splits arrived within a poll and a config edit took up to two minutes.
    //
    // Found 2026-09-22, chasing a drop bag change that would not appear.
    Promise.resolve(hub.data() || hub.load()).then((h) => {
      if (closed) return;
      // A session's proxy if there is one, otherwise the address hub.json
      // gives everybody. This used to require a session, which quietly made
      // the push a feature for signed-in crew only: the spectators, who are
      // the many and the whole reason a fan-out is worth having, went on
      // polling. /live takes no auth and serves public races only, so there
      // was never a reason for the client to insist on one.
      const sess = auth.cfg();
      proxy = (sess && sess.proxyUrl) || hub.proxyUrl();
      if (!h || !h.live || !proxy) return;
      open();
    }).catch(() => { /* no push; the poll underneath is still running */ });
    ...
    function open() {
      ...
      // The message is handed on, where it used to be thrown away. It names
      // which file changed, and the pages use that to decide whether to
      // re-read the config as well as the splits: a config read is cached for
      // a minute on purpose, and this is what says the cache is wrong now.
      // Anything unparseable, or a worker that predates the field, arrives as
      // null, which every caller treats as "something changed, ask again".
      ws.onmessage = (ev) => {
        let msg = null;
        try { msg = JSON.parse(ev && ev.data); } catch (e) { msg = null; }
    ...
    // open() is called above, once hub.json has answered.
    return state;
  }
  ...
  function raceState(cfg, data) {
    ...
    if (everyRunnerDone(cfg, data)) return 'finished';
    const hours = (cfg.cutoffs && cfg.cutoffs.totalHours) || 36;
    if (isFinite(start) && Date.now() > start + hours * 3600000) return 'finished';
    return 'live';
  }

  // Every runner has logged every leg: the strict reading of "the race is over".
  //
  // Separate from raceState because raceState also calls a race finished once
  // the cutoff has passed, and the two disagree about exactly the race that
  // matters: one that blew its cutoff with somebody still walking it in. For
  // deciding how often to poll, the clock is the right answer and being wrong
  // costs nothing. For deciding whether anybody is still out on the course, it
  // is the wrong answer and being wrong swaps the map under them.
  function everyRunnerDone(cfg, data) {
    const runners = (cfg && cfg.runners) || [];
    if (!runners.length || !data) return false;
  ...
  const auth = {
    ...
    // A site admin, which is a different thing from a role on a race. The
    // session carries it because the worker put it there at login; the worker
    // checks it again on every call, so this only decides what to draw.
    isAdmin() {
      if (auth.mode() !== 'proxy') return false;
      const s = auth.loadSession();
  ...
  const api = {
    ...
    mediaDelete: (id)                        => proxyCall('/media/delete', { method: 'POST', body: { id } }),
    ...
    mailStatus:  ()                          => proxyCall('/mail-status'),
    ...
    setVisibility:(slug, visibility)         => proxyCall('/race/visibility', { method: 'POST', body: { slug, visibility } }),
  ...
  // Race files the worker has refused this session, by path. Read by
  // readRaceJson, which would otherwise report a race somebody may not see as
  ...
  const gh = {
    ...
    async getFile(cfg, path) {
      if (cfg.mode === 'proxy') {
        ...
        if (!res.ok) {
          ...
          // A refusal is a sentence the worker already wrote for a person:
          // "not invited to this race". Wrapping it in the method, the path
          // and a slice of JSON turns an answer into a stack trace, and this
          // one reaches the screen: it is what a page shows when it cannot
          // read a race at all.
          let said = null;
          if (res.status === 403) { try { said = JSON.parse(text).error; } catch (e) {} }
          const err = new Error(said || `GET ${path} ${res.status}: ${text.slice(0, 200)}`);
          err.status = res.status; throw err;
        }
        const j = await res.json();
    ...
    async putFile(cfg, path, body, sha, message) {
      if (cfg.mode === 'proxy') {
        ...
        if (!res.ok) {
          const text = await res.text();
          // Same reasoning as the read above: a refusal and a plan limit are
          // both sentences the worker wrote for a person, and both end up in a
          // toast beside a Save button. "PUT races/x/config.json 403: {...}"
          // tells somebody nothing they can act on.
          let said = null;
          if (res.status === 403 || res.status === 402) {
            try { said = JSON.parse(text).error; } catch (e) {}
          }
          const err = new Error(said || `PUT ${path} ${res.status}: ${text.slice(0, 200)}`);
          err.status = res.status; throw err;
        }
        return res.json();
      }
    ...
    async readRaceFile(slug, file, preferWorker) {
      ...
      const keep = (text, opts) => {
        if (text != null) { refusals.delete(path); lastSeen.save(slug, file, text, opts); }
        return text;
      };

      // Set when the worker answered "you may not read this" rather than
      // "there is no such file". The difference matters once every fallback
      // below has also come up empty, which for a private race is what
      // happens: there is no published copy to find, so the caller was told
      // the race did not exist. It does. It is not theirs.
      let refused = null;
      if (preferWorker && cfg) {
        try {
          const r = await gh.getFile(cfg, path);
          if (!r.missing) { offline.note(path, false); return keep(r.content, { source: 'proxy' }); }
        } catch (e) { if (e && e.status === 403) refused = e; /* fall through to Pages */ }
      }
      ...
      if (refused) refusals.set(path, refused); else refusals.delete(path);
      return null;
    },
    ...
    async readRaceJson(slug, file, preferWorker) {
      ...
      if (text == null) {
        // Nothing anywhere, and the worker said why. Its reason is the true
        // one and the only one a person can act on.
        const why = refusals.get(`races/${slug}/${file}`);
        if (why) throw why;
  ...
  // offlineLogging is true on both on purpose. The pricing plan puts it in Pro
  // and it is not gated yet, so today every account has it and the crew screens
  // say so plainly. It is named here anyway so the copy follows the
  // entitlement rather than somebody's memory: the day it is gated, free flips
  // to false and the two screens start telling the truth on their own.
  //
  // Gating it is not a one-line change. See GRANDFATHERED in the worker: it has
  // to go into that list in the same commit, or every account that signed up
  // while it was free quietly loses something it was promised.
  const PLANS = {
    free: { label: 'Free', maxRunnersPerRace: 1, maxCrewPerRace: 2, privateRaces: false, shareLinks: false, offlineLogging: true },
    pro:  { label: 'Pro',  maxRunnersPerRace: null, maxCrewPerRace: null, privateRaces: true, shareLinks: true, offlineLogging: true }
  };
  ...
  const plans = {
    ...
    // The one sentence the pit board and the racer page each say about logging
    // with no signal. Here rather than written into both, because it is a
    // statement about a plan: the day offline logging is gated it has to change
    // in one place, and a page that forgot would be promising something the
    // account does not have at an aid station with no bars.
    //
    // The generous answer when the entitlement is unknown, which matches
    // entitlements() above: the cost of being wrong this way is a sentence,
    // and the cost of being wrong the other way is a crew member who does not
    // trust the thing that is about to save their race.
    offlineNote(ent) {
      return (ent && ent.offlineLogging === false)
        ? 'With no signal a press cannot be held: it needs a connection to ' +
          'land. Holding them on the phone until you have bars is a <b>Pro</b> ' +
          'feature.'
  ...
  // ---------- photos ----------
  // Attached to a leg of the course, taken by the crew, shown on the race page.
  //
  // Three things here that are not obvious from the outside:
  //
  // 1. Every photo is re-encoded through a canvas before it leaves the phone.
  //    That is the resize, and the resize is also the whole of the EXIF
  //    stripping: a canvas knows nothing about the metadata of the image drawn
  //    onto it, so the GPS fix, the timestamp and the camera model do not
  //    survive it. A photo of a runner at an aid station carries that person's
  //    exact coordinates until something takes them off, and this is that
  //    something. It is a side effect of an optimisation, which is a fragile
  //    place for a privacy property to live, so there is a test on it.
  //
  // 2. The queue is IndexedDB, not the localStorage one everything else uses.
  //    That queue holds strings in about five megabytes; a phone photo is
  //    three to five on its own, base64 adds a third, and queue.save swallows
  //    a quota error, so a photo pushed through it would fail silently and a
  //    crew member would believe it was saved. IndexedDB stores the blob
  //    itself and has room.
  //
  // 3. The person taking the photo is standing at an aid station with no
  //    signal, which makes the offline path the normal path rather than the
  //    edge case. Nothing here assumes a connection at any point.
  const MEDIA_DB = 'sendoff-media-v1';
  const MEDIA_STORE = 'queue';
  // The long edge. Big enough to look like a photograph on a laptop, small
  // enough that a spectator on a trailhead LTE bar is not paying for the
  // original: this is the difference between 400KB and 4MB, per photo, per
  // person watching.
  const MEDIA_LONG_EDGE = 1600;
  ...
  function mediaDb() {
    ...
    mediaDbPromise = new Promise((resolve, reject) => {
      let req;
      try { req = indexedDB.open(MEDIA_DB, 1); }
  ...
  function mediaChanged() { for (const fn of mediaListeners) { try { fn(); } catch (e) {} } }

  const media = {
    // Resize and re-encode. Returns { blob, width, height }, always JPEG:
    // every phone camera produces something a canvas can read, and one output
    // type means one thing for the worker to accept.
    //
    // Rejects rather than guesses when the file will not decode, because the
    // alternative is queueing something that can never be sent.
    async prepare(file) {
      ...
      try {
        ...
        const scale = Math.min(1, MEDIA_LONG_EDGE / long);
        ...
        const blob = await new Promise((resolve, reject) => {
          canvas.toBlob(b => b ? resolve(b) : reject(new Error('Could not encode that photo.')),
    ...
    // Put one in the queue. Returns the entry. Sending is a separate step, so
    // a crew member who takes four photos in a row is not waiting on the first.
    async enqueue(slug, entry) {
      const prepared = await media.prepare(entry.file);
      const row = {
        id: (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random()),
        slug,
        legIndex: entry.legIndex,
        runnerId: entry.runnerId || null,
        caption: (entry.caption || '').slice(0, 280),
      ...
      await mediaTx('readwrite', store => store.put(row));
      mediaChanged();
      media.flush().catch(() => {});
      return row;
    },
    ...
    async pending(slug) {
      let all = [];
      try { all = await mediaTx('readonly', store => store.getAll()); } catch (e) { return []; }
      const rows = Array.isArray(all) ? all : [];
      return slug ? rows.filter(r => r.slug === slug) : rows;
    },

    async drop(id) {
      await mediaTx('readwrite', store => store.delete(id));
      mediaChanged();
    },

    onChange(fn) { mediaListeners.push(fn); },

    _flushing: false,
    // Send what is waiting. One at a time and oldest first, because the
    // connection that just came back is one bar at a trailhead and four
    // parallel uploads on it is four failures.
    async flush() {
      if (media._flushing) return;
      const cfg = auth.cfg();
      if (!cfg || cfg.mode !== 'proxy') return;
      media._flushing = true;
      try {
        const rows = (await media.pending()).sort((a, b) => a.at - b.at);
        for (const row of rows) {
          try {
            await media.send(row);
            await media.drop(row.id);
          } catch (err) {
            // 4xx other than 408/429 means this will never succeed: a photo
            // for a race that is gone, or from an account that lost access.
            // Anything else is the signal, and the signal comes back.
            const status = err && err.status;
            if (status && status >= 400 && status < 500 && status !== 408 && status !== 429) {
              row.failed = err.message || 'Rejected';
              await mediaTx('readwrite', store => store.put(row));
              mediaChanged();
            }
            break;
        ...
        media._flushing = false;
      }
    },

    // The upload itself. Raw bytes with the details in the query string: the
    // client has already made exactly one part, and a multipart body would be
    // machinery in the way of somebody with one bar.
    async send(row) {
      const cfg = auth.cfg();
      if (!cfg || cfg.mode !== 'proxy') throw new Error('Sign in required');
      const q = new URLSearchParams({
        id: row.slug,
        leg: String(row.legIndex),
        w: String(row.width || ''),
        h: String(row.height || '')
      });
      if (row.runnerId) q.set('runner', row.runnerId);
      if (row.caption) q.set('caption', row.caption);
      const res = await fetch(cfg.proxyUrl + '/media?' + q.toString(), {
        method: 'POST',
        headers: { Authorization: 'Bearer ' + cfg.session, 'Content-Type': 'image/jpeg' },
        body: row.blob
      });
      let payload = null;
      try { payload = await res.json(); } catch (e) {}
      if (!res.ok) {
        const err = new Error((payload && payload.error) || `Upload failed (${res.status})`);
    ...
    // What a race has. Readable by anyone who can read the race, which for a
    // public one is everybody and for an unlisted one is whoever holds a link.
    // Also answers whether this deployment can take a photo at all. A worker
    // with no bucket bound, and an older worker that has never heard of
    // /media, both have to be told apart from a race nobody has photographed
    // yet: the first two mean do not offer the camera, and an empty array on
    // its own cannot say which.
    //
    // Not academic. The client shipped ahead of the worker once already, and
    // the pit board offered an upload that could only ever fail.
    _configured: null,
    // Remembered per device, because the answer is about the deployment and
    // not about the moment. A crew member who opens the board in a canyon has
    // asked before, from somewhere with signal, and hiding the camera because
    // this particular load could not reach the worker would take it away at
    // exactly the aid station it exists for. Only a yes is remembered: a no
    // costs one successful request to overturn.
    _CONF_KEY: 'sendoff-media-ok-v1',
    _seedConfigured() {
      if (media._configured !== null) return;
      try { if (localStorage.getItem(media._CONF_KEY) === '1') media._configured = true; } catch (e) {}
    },
    async list(slug) {
      const sess = auth.cfg();
      const base = (sess && sess.mode === 'proxy' && sess.proxyUrl) || hub.proxyUrl();
      if (!base) { media._configured = false; return []; }
      const q = new URLSearchParams({ id: slug });
      const t = share.get(slug);
      if (t) q.set('t', t);
      const headers = {};
      if (sess && sess.mode === 'proxy' && sess.session) headers.Authorization = 'Bearer ' + sess.session;
      let res;
      try {
        res = await fetch(String(base).replace(/\/+$/, '') + '/media?' + q.toString(), { headers });
      } catch (e) {
        // No signal. Says nothing either way about the deployment, so the last
        // answer stands rather than turning the camera off in a canyon.
        return [];
      }
      if (res.status === 404 || res.status === 501) {
        media._configured = false;
        try { localStorage.removeItem(media._CONF_KEY); } catch (e) {}
        return [];
      }
      if (!res.ok) return [];
      const payload = await res.json().catch(() => null);
      if (payload && payload.configured === false) { media._configured = false; return []; }
      media._configured = true;
      try { localStorage.setItem(media._CONF_KEY, '1'); } catch (e) {}
      return (payload && payload.media) || [];
    },

    // Null until the first list() comes back. The pit board waits for a real
    // answer rather than assuming either way.
    configured() { media._seedConfigured(); return media._configured; },

    remove(id) { return api.mediaDelete(id); },

    // The whole add-a-photo panel, for any page that wants one.
    //
    // It lived in pit.html until the racer page wanted the same thing, and two
    // copies of a panel is how two screens start disagreeing about what a
    // button does. What stays with the page is the one thing the two know
    // differently: where the race is, which decides the leg it opens on.
    //
    // ctx: { slug, cfg(), defaultLeg(), canAdd() }
    //
    // Everything is looked up inside the host rather than by document id, so
    // two of these on one page would not fight over the same select.
    mountPanel(host, ctx) {
      if (!host || !ctx || !ctx.slug) return null;

      let rows = [], queued = [], open = false, built = false;
      let legSig = '', runnerSig = '', thumbUrls = [], sayTimer = null;
      const q = (sel) => host.querySelector(sel);

      // Built once and updated in place, never rebuilt. Tapping Add photo
      // opens the operating system's picker, the picker stays up for longer
      // than a poll, and a poll that replaced the file input would hand the
      // photograph back to an element no longer in the document: the change
      // event fires on a detached node, reaches nobody, and the whole thing is
      // silent. That shipped once. Nothing in the body is replaced.
      function build() {
        host.innerHTML =
          '<button type="button" class="photo-head" data-ph-toggle aria-expanded="false">' +
            '<span>Photos</span>' +
            '<span class="photo-head-right"><span data-ph-summary></span>' +
            '<span class="photo-head-caret"></span></span>' +
          '</button>' +
          '<div class="photo-body" data-ph-body hidden>' +
            '<div class="photo-row">' +
              '<select data-ph-leg aria-label="Which leg"></select>' +
              '<select data-ph-runner aria-label="Who is in it"></select>' +
            '</div>' +
            '<div class="photo-row">' +
              '<input type="text" data-ph-caption maxlength="280" placeholder="A line about it (optional)">' +
              // The camera first, and solid rather than dashed, because it is
              // the one that works where this gets used. An aid station with no
              // signal is exactly where the gallery picker cannot show anything:
              // most of a phone's library is in the cloud, and the picker
              // answers "Can't load some Photos" and hands nothing back.
              // capture goes to the camera app, which never needed a network.
              // No `multiple` on it, because capture takes one at a time.
              '<label class="photo-pick primary">Take photo' +
                '<input type="file" data-ph-camera accept="image/*" capture="environment"></label>' +
              '<label class="photo-pick">Choose' +
                '<input type="file" data-ph-file accept="image/*" multiple></label>' +
            '</div>' +
            '<div class="photo-state" data-ph-state></div>' +
            '<div class="photo-thumbs" data-ph-thumbs></div>' +
          '</div>';
        wire();
      }

      // Says something, and stops the poll writing over it for a moment.
      function say(text, cls) {
        const el = q('[data-ph-state]');
        if (!el) return;
        el.className = 'photo-state' + (cls ? ' ' + cls : '');
        el.textContent = text;
        el.dataset.sticky = '1';
        clearTimeout(sayTimer);
        sayTimer = setTimeout(() => { delete el.dataset.sticky; render(); }, 4000);
      }

      function wire() {
        host.addEventListener('click', (e) => {
          if (!e.target.closest('[data-ph-toggle]')) return;
          open = !open;
          render();
          // Straight into the leg picker: three controls, and the first is the
          // one that gets changed.
          if (open) { const l = q('[data-ph-leg]'); if (l) l.focus(); }
        });

        host.addEventListener('change', async (e) => {
          const input = e.target.closest('[data-ph-file], [data-ph-camera]');
          if (!input) return;
          const files = [...(input.files || [])];
          input.value = '';
          if (!files.length) return;
          const legIndex = parseInt(q('[data-ph-leg]').value, 10);
          const runnerId = q('[data-ph-runner]').value || null;
          const caption = q('[data-ph-caption]').value || '';
          say(files.length > 1 ? `Preparing ${files.length} photos…` : 'Preparing…');
          let added = 0;
          for (const file of files) {
            try {
              await media.enqueue(ctx.slug, { legIndex, runnerId, caption, file });
              added++;
            } catch (err) {
              say(err.message || 'Could not add that photo.', 'err');
              break;
            }
          }
          const cap = q('[data-ph-caption]');
          if (cap) cap.value = '';
          await refresh();
          // After the refresh, so it is the last word rather than the first. A
          // photo that queued with no signal used to look exactly like one that
          // had never been taken.
          if (added) {
            const waiting = queued.filter(x => !x.failed).length;
            say(waiting
        ...
        host.addEventListener('click', async (e) => {
          const drop = e.target.closest('[data-ph-drop]');
          if (drop) { await media.drop(drop.dataset.phDrop); return refresh(); }
          const kill = e.target.closest('[data-ph-remove]');
          if (!kill) return;
          if (!confirm('Delete this photo? It comes off the race page for everybody.')) return;
          try { await media.remove(kill.dataset.phRemove); } catch (err) {}
          refresh();
        });

        // A photo that went out while the page was open should stop looking
        // like one that is waiting and start looking like one that landed.
        media.onChange(() => { refresh(); });
      }

      function drawThumbs() {
        const box = q('[data-ph-thumbs]');
        if (!box) return;
        // One object URL per queued photo per render, never released, on a page
        // that renders every few seconds for as long as a hundred miler.
        for (const u of thumbUrls) { try { URL.revokeObjectURL(u); } catch (e) {} }
        thumbUrls = [];
        const waitingHtml = queued.map(x => {
          const u = URL.createObjectURL(x.blob);
          thumbUrls.push(u);
          return `<figure class="queued"><img alt="Waiting to send" src="${u}">` +
            `<button class="kill" type="button" data-ph-drop="${esc(x.id)}" aria-label="Discard">×</button>` +
            `</figure>`;
        }).join('');
        const sentHtml = rows.slice().reverse().slice(0, 12).map(m =>
          `<figure><img alt="${esc(m.caption || 'Race photo')}" src="${esc(m.url)}">` +
          `<button class="kill" type="button" data-ph-remove="${esc(m.id)}" aria-label="Delete">×</button>` +
          `</figure>`).join('');
        box.innerHTML = waitingHtml + sentHtml;
        box.hidden = !(waitingHtml || sentHtml);
      }

      function render() {
        const cfg = ctx.cfg && ctx.cfg();
        if (!cfg) return;
        // Whoever can write the race can add to it. Everything the roadmap says
        // about moderation starts the moment somebody else can.
        if (!ctx.canAdd()) { host.style.display = 'none'; return; }
        // And only where a photo can actually land: this shipped ahead of the
        // worker that takes them once, and offered a camera whose every upload
        // could only fail.
        if (media.configured() !== true) { host.style.display = 'none'; return; }
        host.style.display = '';
        if (!built) { built = true; build(); }

        host.className = 'photo-bar' + (open ? '' : ' collapsed');
        q('[data-ph-body]').hidden = !open;
        q('[data-ph-toggle]').setAttribute('aria-expanded', String(open));

        // The selects, only when what goes in them changed: rebuilding on the
        // poll would reset a choice somebody had just made.
        const legs = course.legs(cfg) || [];
        const sig = legs.map(l => l.index + ':' + (l.toAid || '')).join('|');
        const legSel = q('[data-ph-leg]');
        if (sig !== legSig) {
          legSig = sig;
          const keep = legSel.value || String(ctx.defaultLeg());
          legSel.innerHTML = legs.map(l =>
            `<option value="${l.index}">Leg ${l.index}${l.toAid ? ' · ' + esc(l.toAid) : ''}</option>`).join('');
          legSel.value = keep;
          if (!legSel.value) legSel.value = String(ctx.defaultLeg());
        }
        const runners = cfg.runners || [];
        const rsig = runners.map(r => r.id + ':' + (r.name || '')).join('|');
        const runnerSel = q('[data-ph-runner]');
        if (rsig !== runnerSig) {
          runnerSig = rsig;
          const keep = runnerSel.value;
          runnerSel.innerHTML = '<option value="">Nobody in particular</option>' +
            runners.map(r => `<option value="${esc(r.id)}">${esc(r.name || r.id)}</option>`).join('');
          runnerSel.value = keep;
        }

        const waiting = queued.filter(x => !x.failed).length;
        const stuck = queued.filter(x => x.failed);

        // What the head says while the body is shut. The count waiting for
        // signal has to be one of them: somebody with three unsent pictures
        // should not have to open anything to find that out.
        const summary = q('[data-ph-summary]');
        summary.textContent = stuck.length ? `${stuck.length} stuck`
          : waiting ? `${waiting} waiting for signal`
          : `${rows.length} on this race`;
        summary.className = stuck.length ? 'photo-head-err' : waiting ? 'photo-head-warn' : '';

        const state = q('[data-ph-state]');
      ...
      async function refresh() {
        try { rows = await media.list(ctx.slug); } catch (e) { rows = []; }
        try { queued = await media.pending(ctx.slug); } catch (e) { queued = []; }
        render();
      }

      return { render, refresh, host };
    },

    // Grouped the way both views want it: the leg's dropdown asks for one, the
    // gallery walks them in order.
    byLeg(rows) {
      const out = new Map();
      for (const r of rows || []) {
        if (!out.has(r.legIndex)) out.set(r.legIndex, []);
        out.get(r.legIndex).push(r);
      }
      return out;
    }
  };

  // Anything taken with no signal goes out when there is some, and the moments
  // worth trying are the ones the split queue already settled on: the page
  // opens, the phone comes back to the foreground out of a vest pocket, and a
  // slow tick for when the browser fires neither.
  //
  // The `online` event on its own was not enough, which is what "it took a
  // couple of refreshes" turned out to mean. Coming off airplane mode does not
  // reliably fire it on a phone, and when it does fire it can arrive before the
  // radio is actually carrying anything, so the one retry it triggers fails and
  // nothing tries again. The photograph then sits in the queue until somebody
  // reloads the page, which is not a thing to ask of a crew member at an aid
  // station.
  //
  // flush() is a no-op with nothing waiting, so none of this costs anything on
  // a race where the signal held.
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    window.addEventListener('online', () => { media.flush().catch(() => {}); });
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) media.flush().catch(() => {});
    });
    setInterval(() => { media.flush().catch(() => {}); }, 30000);
  }
  ...
  // 'admin' is not a role on a race and can never be granted as one: the worker
  // puts it on the config it hands back when a site admin is working a race
  // they are not on, so that a page knows it may save rather than drawing a
  // form every press of which will be refused. It is a writing role and it is
  // deliberately not 'owner': deleting a race, and handing out access to it,
  // stay with the person whose race it is.
  const WRITING_ROLES = new Set(['owner', 'crew', 'racer', 'pacer', 'admin']);

  const roles = {
    all: ROLES,
    label(key) {
      if (key === 'owner') return 'Creator';
      if (key === 'admin') return 'Site admin';
      return ROLE_LABEL[key] || 'Viewer';
    },
  ...
  const fuel = {
    ...
    // board, and the one-tap items, because a flask is a flask whoever drinks it.
    //
    ...
    // Item editor rows in, config-shaped items out, filtered against the
    // metrics that survived the same save. Removing Fluid should not leave
    // every item carrying an orphaned fluid number.
    //
    // An item with no numbers at all is kept, and that is the point of it. A
    // medicine is a thing you need to know was taken and when, and it has no
    // calories and no fluid: before this, the only way to record one was to
    // type it into the Meds box every time. Now it is a button, and what it
    // records is that it was taken.
    //
    // So the name is the whole of what is required. Numbers are what an item
    // adds, not what makes it one.
    ...
    // What was tapped on a leg, as words: "Ibuprofen \u00d72 \u00b7 Gel".
    //
    // The counts are the only record an item with no numbers leaves, so
    // without this a medicine would be logged into a field nobody displays.
    // Shared between the race page and the printout rather than written twice,
    // because the printout is what somebody reads after a race trying to work
    // out when a pill was given.
    //
    // Keyed by position, which is how the counts are stored. Deleting an item
    // therefore renames history: counts under it become the next item's. Worth
    // knowing before reordering a list mid-race; appending is safe, which is
    // what anybody does during one.
    itemsTaken(cfg, leg) {
      if (!leg) return [];
      return fuel.presets(cfg).map((p, i) => {
        const n = +leg['preset_' + i] || 0;
        return n > 0 ? (n > 1 ? `${p.name} \u00d7${n}` : p.name) : null;
      }).filter(Boolean);
    },
  ...
  function openProfileModal() {
    ...
    m.body.innerHTML =
      ...
      '<div id="prof-form" style="display:none">' +
        '<label class="modal-field"><span>First name</span>' +
          '<input type="text" id="prof-first" autocomplete="given-name" placeholder="e.g. Jason" /></label>' +
        '<label class="modal-field"><span>Last name</span>' +
          '<input type="text" id="prof-last" autocomplete="family-name" placeholder="e.g. Dupree" /></label>' +
        '<label class="modal-field"><span>Name your crew will see</span>' +
          '<input type="text" id="prof-name" autocomplete="nickname" placeholder="e.g. Jason" /></label>' +
        '<p class="modal-hint">Leave the last one blank and your crew sees your ' +
          'first and last name. Fill it in when the board should say something ' +
          'else: a nickname, or just a first name where two people share one.</p>' +
    ...
    api.profile().then(({ profile }) => {
      m.body.querySelector('#prof-first').value = profile.firstName || '';
      m.body.querySelector('#prof-last').value = profile.lastName || '';
      ...
      m.body.querySelector('#prof-first').focus();
    }).catch(err => {
      loading.textContent = '';
    ...
    m.primary.addEventListener('click', async () => {
      ...
      try {
        await api.saveProfile({
          firstName: m.body.querySelector('#prof-first').value,
          lastName: m.body.querySelector('#prof-last').value,
  ...
  // ---------- the tutorial ----------
  // What a screen says the first time somebody lands on it.
  //
  // One store, one banner, and a menu item that reopens it, because the
  // alternative is fifteen screens each growing their own version of this and
  // none of them agreeing on where the text sits or how it goes away.
  //
  // Deliberately not a modal. The hub has one and it earns it: somebody
  // arriving there is deciding whether to use SendOff at all. Every other
  // screen is somebody who has already decided, and on the pit board it is
  // somebody with a runner in front of them. A door in front of that screen is
  // worse than no teaching at all.
  //
  // What it does not do: it holds no copy of its own and knows nothing about
  // roles. A page passes the words, so a page that shows crew and viewers
  // different screens can pass different words, and the store stays a store.
  const tutorial = (function () {
    const KEY = 'race-hub-tutorial-v1';
    // Last resort. Storage throws in private mode on some engines, and a
    // teaching banner is never worth breaking a page for: with no storage the
    // screen teaches once per page load and forgets, which is the right way
    // round to fail.
    let mem = {};
    let current = null;
    ...
    function load() {
      ...
      try { const raw = sessionStorage.getItem(KEY); if (raw) return JSON.parse(raw) || {}; } catch (e) {}
      return mem;
    }
    function save(obj) {
      mem = obj;
      try { localStorage.setItem(KEY, JSON.stringify(obj)); return; } catch (e) {}
      try { sessionStorage.setItem(KEY, JSON.stringify(obj)); } catch (e) {}
    }

    // A number per screen rather than a flag, so rewriting one screen's copy
    // shows it again to people who dismissed the old words without resetting
    // every other screen too. Bumping KEY resets the lot.
    function seen(key, version) {
      const v = load()[key];
      return typeof v === 'number' && v >= (version || 1);
    }
    function markSeen(key, version) {
      const all = load();
      all[key] = Math.max(all[key] || 0, version || 1);
      save(all);
    }
    function forget(key) {
      const all = load();
      if (key) delete all[key]; else for (const k of Object.keys(all)) delete all[k];
      save(all);
    }

    function render(cfg) {
      const host = typeof cfg.where === 'string' ? document.querySelector(cfg.where) : cfg.where;
      if (!host) return null;
      // Reopening from the menu replaces rather than stacks.
      const old = document.querySelector('.tut');
      if (old) old.remove();
      ...
      el.innerHTML =
        '<div class="tut-head">' +
          ...
          '<button type="button" class="tut-close">Got it</button>' +
        '</div>' +
        '<ol class="tut-points"></ol>';
      el.querySelector('.tut-title').textContent = cfg.title || 'How this page works';
      const list = el.querySelector('.tut-points');
      for (const point of cfg.points || []) {
        const li = document.createElement('li');
        // Markup on purpose: a point whose whole job is to name a button reads
        // better with the button's name set in the type the button uses. The
        // copy is ours, written here, not anything a race carries.
        li.innerHTML = point;
        list.appendChild(li);
      }
      el.querySelector('.tut-close').addEventListener('click', () => {
        markSeen(cfg.key, cfg.version);
        el.remove();
      });
      // First thing in whatever it was given. A page that wants it somewhere
      // other than the top of a section passes an empty element sitting where
      // it wants it, which is more predictable than a position argument.
      host.insertBefore(el, host.firstChild);
      return el;
    }

    return {
      // Called by a page once it knows what it is. Shows the banner if this
      // screen has not been seen, and puts the menu item up either way.
      register(cfg) {
        if (!cfg || !cfg.key) return null;
        current = cfg;
        // The header mounts on DOMContentLoaded and a page registers after
        // that, so the menu was already built without the item in it.
        if (accountWidgetRender) accountWidgetRender();
        // A screen that already has its own first-visit thing registers to get
        // the menu item and nothing else. The hub is the one: it opens a full
        // dialog, which it earns, and which until now could never be seen a
        // second time by anybody who pressed Look around first.
        if (typeof cfg.open === 'function') return null;
        if (seen(cfg.key, cfg.version)) return null;
        return render(cfg);
      },
      // The menu item. Shows it again whether or not it has been seen, and
      // marks it seen, since it now has been.
      open() {
        if (!current) return null;
        if (typeof current.open === 'function') return current.open();
        const el = render(current);
        markSeen(current.key, current.version);
        return el;
      },
      // Put it away without anybody pressing Got it. The pit board calls this
      // on the first press: somebody who has started working has read it or
      // decided not to, and either way a banner between them and the next
      // racer is in the way. Nothing to dismiss is not a failure.
      dismiss() {
        const el = document.querySelector('.tut');
        if (!el) return false;
        el.remove();
        if (current) markSeen(current.key, current.version);
        return true;
      },
      has() { return !!current; },
      seen, markSeen, forget
    };
  })();
  ...
  function mountAccountWidget() {
    ...
    function render() {
      ...
      if (!signedIn && !navLinks.length && !tutorial.has()) {
        // Nowhere to go and one thing to do: a menu to hold it would be a menu
        // of one. A screen that has something to teach makes it two, and the
        // people who most need the teaching are the ones with no account.
      ...
      bar.innerHTML =
        ...
        '<div class="account-menu" role="menu" hidden>' +
          ...
          // Above the account items and below the page links, because it is
          // about the page, not about you.
          (tutorial.has()
            ? '<button type="button" class="account-menu-item" role="menuitem" data-act="tutorial">How this page works</button>'
            : '') +
          (signedIn
      ...
      menu.addEventListener('click', (e) => {
        ...
        closeMenu(false);
        if (act === 'tutorial') tutorial.open();
        else if (act === 'feedback') openFeedbackModal();
  ...
  // Saving used to be announced by a strip at the bottom of the screen: three
  // seconds of "Race details saved." a long way from the button that had just
  // been pressed. On a phone, with the keyboard closing and the page settling
  // under it, it was easy to press Save and never see that anything happened.
  //
  // So the button reports on itself. Save changes, Saving, Saved, and back to
  // Save changes. It is the thing the finger is already on and the thing the
  // eye is already on, and it cannot be missed by looking at the wrong part of
  // the screen.
  //
  // Failure is not reported here. What went wrong needs a sentence, and a
  // button is one or two words wide, so fail() puts the button back as it was
  // and the message goes where messages go.
  const saveBtn = {
    // How long "Saved" stays up. Long enough to be seen by somebody who
    // glanced away while it was in flight; short enough that nobody is waiting
    // on it, since the button is live again the whole time.
    DONE_MS: 2200,

    // WeakMaps rather than a property on the element: a redraw throws the
    // button away and these go with it, which is what should happen. A page
    // that rebuilds its buttons passes a selector instead and gets the state
    // put back on whatever is there now.
    _orig: new WeakMap(),
    _timer: new WeakMap(),

    _el(btn) { return typeof btn === 'string' ? document.querySelector(btn) : btn; },

    // The label it goes home to, remembered once. Pressing Save twice in quick
    // succession must not leave "Saving" as the label to go home to.
    _remember(el) {
      if (!saveBtn._orig.has(el)) saveBtn._orig.set(el, el.textContent);
      clearTimeout(saveBtn._timer.get(el));
      return saveBtn._orig.get(el);
    },

    busy(btn, label) {
      const el = saveBtn._el(btn);
      if (!el) return;
      saveBtn._remember(el);
      // Pinned to the width it had while it was still saying its own name.
      // "Save changes", "Saving" and "Saved" are three different lengths, and
      // without this the button shrinks and grows twice on every save, which
      // on a right-aligned row drags the button out from under the finger
      // that is still on it.
      if (el.offsetWidth) el.style.minWidth = el.offsetWidth + 'px';
      el.disabled = true;
      el.classList.remove('is-saved');
      el.classList.add('is-busy');
      el.textContent = label || 'Saving\u2026';
    },

    // Enabled again straight away, deliberately. Somebody who changed
    // something else while this was in flight should not have to wait out a
    // confirmation to save that too.
    done(btn, label) {
      const el = saveBtn._el(btn);
      if (!el) return;
      const orig = saveBtn._remember(el);
      // Pinned here as well as in busy(), and not only for tidiness: a page
      // that rebuilds its buttons hands this a brand new element that never
      // saw busy() and carries no width, so without this the pit board's
      // Save intake shrank to fit the word "Saved" the moment it landed.
      // On an element busy() already pinned this measures the pinned width
      // and sets it again, which changes nothing.
      if (el.offsetWidth) el.style.minWidth = el.offsetWidth + 'px';
      el.disabled = false;
      el.classList.remove('is-busy');
      el.classList.add('is-saved');
      el.textContent = label || 'Saved';
      saveBtn._timer.set(el, setTimeout(() => {
        el.classList.remove('is-saved');
        el.textContent = orig;
        el.style.minWidth = '';
      }, saveBtn.DONE_MS));
    ...
    fail(btn) {
      ...
      const orig = saveBtn._remember(el);
      el.disabled = false;
      el.classList.remove('is-busy', 'is-saved');
      el.textContent = orig;
      el.style.minWidth = '';
    }
  };
  ...
  // Who can find a race, as opposed to who can open it. Listed puts it on the
  // hub and in the public manifest; private takes it off both, takes down its
  // share page, moves the GPX out of the public repository, and closes every
  // read to the people on the race.
  //
  // The creator or a site admin, matching the endpoint. It lives next to
  // Delete because it is a decision about the race's existence in public, not
  // a setting the crew adjusts while working.
  function mountVisibility(host, ctx) {
    if (!host || !ctx || !ctx.slug) return null;

    const state = () => {
      const cfg = ctx.cfg ? ctx.cfg() : null;
      return (cfg && cfg.visibility) === 'private' ? 'private' : 'public';
    };

    // What the panel was last built for. The settings page reloads itself
    // every thirty seconds and redraws from what comes back, and without this
    // that would throw away a choice somebody was part way through making, or
    // wipe "Saved" off the button a moment after it appeared. A redraw is only
    // for a race whose stored visibility has actually moved.
    let drawnAs = null;
    // Whether unlisting is out of reach on this plan, and which plan that is.
    // False until the answer arrives, because the panel should not wait on a
    // request to appear and the generous guess costs a sentence at worst: the
    // worker refuses anything the plan does not allow whatever this says.
    let gated = false;
    let planLabel = '';

    function draw() {
      const now = state();
      if (drawnAs === now && host.querySelector('#ax-vis')) return;
      drawnAs = now;
      // The trap worth naming. A race already private on a plan that cannot
      // make one private still shows as private, and the worker will take it
      // public;
      // what it will not do is give it back. Saying so beforehand is the whole
      // point of greying the option out rather than refusing at the press.
      // A public race can be unlisted and cannot be made private, which is the
      // one thing somebody pressing this needs to know and cannot see. While a
      // race is public its hub entry, share page, preview card and course file
      // are committed to a public repository; this takes them out of the
      // current files and starts checking who is asking, and cannot take them
      // out of that repository's history.
      // What was published while this race was on the hub, said before the
      // press and again afterwards. The second half is the one that took a
      // correction: a race reading as plainly "private" the moment it flips is
      // the comfortable answer and the wrong one, and the warning used to
      // vanish at exactly the moment it became a statement of fact.
      const cfgNow = ctx.cfg ? ctx.cfg() : null;
      const everPublic = !!(cfgNow && cfgNow.everPublic);
      const KEPT =
        'its hub entry, share card and course file carry the race name, the date, ' +
        'the place and the racers\' names, and they stay in a public repository\'s ' +
        'history. Splits, notes, photos and the roster were never published and are ' +
        'not affected.';
      const publishedNote =
        now === 'public'
          ? '<div class="access-help" style="border-left-color: var(--yellow)">' +
            'This race is on the hub, so it has already been published. Taking it off ' +
            'stops it being listed and closes it to everyone but the people on it, ' +
            'which is worth doing. What it cannot do is take back what was published: ' +
            KEPT + ' A race that has to be private has to be made private when ' +
            'it is created.</div>'
        : everPublic
          ? '<div class="access-help" style="border-left-color: var(--yellow)">' +
            'This race was on the hub before, so it is unlisted rather than fully ' +
            'private. It is closed now and only the people on it can open it, and ' +
            KEPT + '</div>'
          : '';
      const gateNote = !gated ? '' :
        '<div class="access-help" style="border-left-color: var(--yellow)">' +
        `Private races are a <b>Pro</b> feature. You are on the ${esc(planLabel)} plan, ` +
        'so it can be listed on the hub but not taken off it' +
        (now === 'private'
          ? '. It is private today, and moving it to the hub on this plan cannot be undone.'
          : '.') +
        '</div>';
      host.innerHTML =
        '<div class="access-help">Listed puts the race on the hub, where anyone ' +
        'can find it and follow along without an account. Private takes it off ' +
        'the hub, takes down its share card, moves the course file out of public ' +
        'storage, and opens it only for the people on it. Crew log the race ' +
        'either way. ' +
        '<a href="/privacy.html#public" target="_blank" rel="noopener">What ' +
        'private means</a>.</div>' +
        publishedNote +
        gateNote +
        '<div class="access-add" style="margin-top:10px">' +
          '<select id="ax-vis">' +
            `<option value="public"${now === 'public' ? ' selected' : ''}>Listed on the hub</option>` +
            `<option value="private"${now === 'private' ? ' selected' : ''}` +
              `${gated && now !== 'private' ? ' disabled' : ''}>Private</option>` +
          '</select>' +
          '<button class="btn" id="ax-vis-save">Save</button>' +
        '</div>' +
        '<div id="ax-vis-result"></div>';

      const sel = host.querySelector('#ax-vis');
      const btn = host.querySelector('#ax-vis-save');
      const out = host.querySelector('#ax-vis-result');

      btn.addEventListener('click', async () => {
        const want = sel.value;
        out.innerHTML = '';
        if (want === state()) { saveBtn.done(btn, 'No change'); return; }
        saveBtn.busy(btn);
        try {
          const res = await api.setVisibility(ctx.slug, want);
          // The config is the fact; the manifest and the course follow it. A
          // half-done move is worth naming rather than swallowing, because the
          // race did change and an operator can act on what is left.
          const left = [];
          if (res && res.manifestErr) left.push('the hub listing');
          if (res && res.courseErr) left.push(res.courseErr);
          if (res && res.shareErr) left.push(res.shareErr);
          const cfg = ctx.cfg ? ctx.cfg() : null;
          if (cfg) cfg.visibility = want;
          // So the next reload's redraw is a no-op rather than rebuilding the
          // panel and taking the confirmation off the button with it.
          drawnAs = want;
          if (left.length) {
            out.innerHTML = `<div class="access-err">Saved, but: ${esc(left.join('; '))}.</div>`;
            saveBtn.fail(btn);
          } else {
            saveBtn.done(btn);
          }
          if (ctx.onChanged) ctx.onChanged(want);
        } catch (err) {
          out.innerHTML = `<div class="access-err">${esc(err.message)}</div>`;
          saveBtn.fail(btn);
        }
    ...
    // The gate the worker applies is the race owner's, and here that is the
    // person reading this: the only other people who see this panel are site
    // admins, who are not gated at all because this is the moderation lever.
    //
    // Asked after the first draw rather than before it, so the panel is on
    // screen either way. The answer is cached for the page's life, so the
    // thirty-second reload does not ask again.
    if (!auth.isAdmin()) {
      plans.entitlements().then((ent) => {
        if (!ent || ent.privateRaces !== false) return;
        gated = true;
        planLabel = plans.label(ent.plan);
        // The one redraw this is allowed, and only while the select still
        // shows what the race is. A slow answer landing after somebody has
        // already picked something must not take the pick away from them; the
        // worker refuses what the plan does not allow either way.
        const sel = host.querySelector('#ax-vis');
        if (sel && sel.value !== state()) return;
        drawnAs = null;
        draw();
      }).catch(() => { /* the worker is still the authority */ });
    }

    return { host, redraw: draw };
  }
  ...
  const access = {
    ...
    mountVisibility: mountVisibility,
    mount(section, ctx) {
  ...
  AccessPanel.prototype.render = async function () {
    ...
    // A private race that was on the hub before is unlisted, not private: what
    // was published while it was listed is in a public repository's history.
    const wasPublished = !!(this.cfg() || {}).everPublic;
    ...
    this.body().innerHTML = `
      ...
      <div class="access-section">
        ...
        <div class="access-hint">${isPrivate
          ? (wasPublished
            ? 'This race is unlisted rather than fully private: it was on the hub before, and its hub entry, share card and course file stay in a public repository\'s history. It is closed now, and this address opens it only for the people on it. A view link below lets somebody watch without an account.'
            : 'This race is private. It is kept off the hub and out of search, and this address opens it only for the people on it: SendOff checks who is asking every time, and answers anybody else as it would a race that does not exist. A view link below lets somebody watch without an account.')
          : (pageUrl === cardUrl
        ...
        ` : ((shareLinksUnknown || (s.shareLinks || []).length) ? `
          <div class="access-list">${shareLinksHtml}</div>
          <div class="access-hint">These were made while the race was private. They do nothing while it is public, and start working again if you make it private.</div>
        ` : '')}
      </div>
  ...
  // ---------- expose ----------
  window.Race = { fmt, units, slug, qs, raceHref, poll, raceState, everyRunnerDone, POLL_MS, live, hub, config, auth, share, api, gh, queue, roles, plans, archive, activities, nav, course, stationAt, fuel, compute, gpx, charts, access, mountAccountWidget, mountBetaBadge, offline, lastSeen, feedback, openFeedbackModal, openSigninModal, tutorial, media, saveBtn };
})();
