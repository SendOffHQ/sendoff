...
(function () {
  ...
  const gh = {
    ...
    async readRaceFile(slug, file, preferWorker) {
      ...
      const keep = (text) => { if (text != null) lastSeen.save(slug, file, text); return text; };
      ...
      try {
        ...
        if (res.ok) { offline.noteResponse(res); return keep(await res.text()); }
  ...
  const lastSeen = {
    ...
    save(slug, file, text) {
      ...
      const row = JSON.stringify({ at: new Date().toISOString(), text });
      try {
