...
(function () {
  ...
  // hub and in the public manifest; unlisted takes it off both, takes down its
  // share page and moves the GPX out of the public repository.
  //
  ...
  function mountVisibility(host, ctx) {
    ...
    function draw() {
      ...
      drawnAs = now;
      // The trap worth naming. A race already unlisted on a plan that cannot
      // unlist one still shows as unlisted, and the worker will take it public;
      ...
      const gateNote = !gated ? '' :
        ...
        `Unlisted races are a <b>Pro</b> feature. You are on the ${esc(planLabel)} plan, ` +
        'so it can be listed on the hub but not taken off it' +
        (now === 'private'
          ? '. It is unlisted today, and moving it to the hub on this plan cannot be undone.'
          : '.') +
        '</div>';
      host.innerHTML =
        '<div class="access-help">Listed puts the race on the hub, where anyone ' +
        'can find it and follow along without an account. Unlisted takes it off ' +
        'the hub, takes down its share card and moves the course file out of ' +
        'public storage. The address does not change, so a race that has already ' +
        'been public may still sit in somebody\'s history: unlisting hides it from ' +
        'here on rather than taking back what was shared. Crew log the race either ' +
        'way. <a href="/privacy.html#public" target="_blank" rel="noopener">What ' +
        'unlisted means</a>.</div>' +
        gateNote +
        '<div class="access-add" style="margin-top:10px">' +
          '<select id="ax-vis">' +
            `<option value="public"${now === 'public' ? ' selected' : ''}>Listed on the hub</option>` +
            `<option value="private"${now === 'private' ? ' selected' : ''}` +
              `${gated && now !== 'private' ? ' disabled' : ''}>Unlisted</option>` +
          '</select>' +
